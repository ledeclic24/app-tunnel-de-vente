    ...
    <div className={`bg-background border rounded-[1.5rem] p-4 transition-colors ${selected ? 'border-accent' : 'border-surface/10'}`}>
          ...
          {editing ? (
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <button onClick={saveEdit} className="hover-lift text-xs font-semibold text-accent">Enregistrer</button>
                <button onClick={() => setEditing(false)} className="hover-lift text-xs text-surface/50">Annuler</button>
              </div>
              <div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <button
          ...
          disabled={generating}
          className="hover-lift inline-flex items-center gap-1.5 text-xs font-semibold text-accent disabled:opacity-50"
        >
          {generating ? (
    ...
    <div className="max-w-2xl mx-auto">
            ...
            <div className="flex flex-wrap gap-2">
              {TONES.map((t) => (
                <button
                  ...
                  className={`hover-lift px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${ebook.tone === t.key ? 'bg-primary text-background' : 'bg-primary/5 text-surface/60'}`}
                >
                  {t.label}
                </button>
              ))}
            ...
            <div className="flex flex-wrap gap-2">
              {LANGUAGES.map((l) => (
                <button
                  ...
                  className={`hover-lift px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${(ebook.language || 'fr') === l.key ? 'bg-primary text-background' : 'bg-primary/5 text-surface/60'}`}
                >
                  {l.label}
                </button>
              ))}
        ...
        <div className="bg-background border border-surface/10 rounded-[1.5rem] p-4 mb-6 space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {TONES.map((t) => (
              <button
                ...
                className={`hover-lift px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${newTone === t.key ? 'bg-primary text-background' : 'bg-primary/5 text-surface/60'}`}
              >
                {t.label}
              </button>
            ))}
          ...
          <div className="flex gap-2">
            <button
              ...
              className="hover-lift text-xs font-semibold text-accent disabled:opacity-50"
            >
              {regeneratingAll ? 'Régénération en cours...' : 'Changer le ton et tout régénérer'}
            </button>
            <button type="button" onClick={() => setShowChangeTone(false)} disabled={regeneratingAll} className="hover-lift text-xs text-surface/50 inline-flex items-center gap-1 disabled:opacity-50">
      ...
      {selectedIds.size > 0 && (
        <div className="flex flex-wrap items-center gap-3 bg-accent/5 border border-accent/20 rounded-2xl px-4 py-3 mb-3">
          ...
          <button onClick={handleBulkGenerate} disabled={bulkBusy || Boolean(generatingId)} className="hover-lift inline-flex items-center gap-1.5 text-xs font-semibold text-accent disabled:opacity-50">
            <Wand2 className="w-3.5 h-3.5" /> Générer le contenu
          </button>
          <button onClick={handleBulkRegenerate} disabled={bulkBusy} className="hover-lift inline-flex items-center gap-1.5 text-xs font-semibold text-accent disabled:opacity-50">
      ...
      {showAddChapter ? (
        <form onSubmit={handleAddChapter} className="bg-background border border-surface/10 rounded-[1.5rem] p-4 mt-4 space-y-2">
          ...
          <div className="flex gap-2">
            <button type="submit" className="hover-lift text-xs font-semibold text-accent">Ajouter</button>
            <button type="button" onClick={() => setShowAddChapter(false)} className="hover-lift text-xs text-surface/50 inline-flex items-center gap-1"><X className="w-3 h-3" /> Annuler</button>
          </div>
        </form>
      ) : (
        <button
      ...
      {showGenerateMore ? (
        <form onSubmit={handleGenerateMoreChapters} className="bg-background border border-surface/10 rounded-[1.5rem] p-4 mt-3 space-y-2">
          ...
          />
          <div className="flex gap-2">
            <button type="submit" disabled={bulkBusy} className="hover-lift text-xs font-semibold text-accent disabled:opacity-50">
              {bulkBusy ? 'Génération...' : 'Proposer'}
            </button>
            <button type="button" onClick={() => setShowGenerateMore(false)} className="hover-lift text-xs text-surface/50 inline-flex items-center gap-1"><X className="w-3 h-3" /> Annuler</button>
          </div>
        </form>
      ) : (
        <button
