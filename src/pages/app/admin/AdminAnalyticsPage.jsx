...
function Delta({ current, previous }) {
  ...
  if (diff === 0) {
    return <span className="inline-flex items-center gap-0.5 text-xs text-background/40"><Minus className="w-3 h-3" /> stable</span>;
  }
  const up = diff > 0;
  return (
          ...
          return (
            <div key={key}>
              <div className="flex justify-between text-sm mb-1">
                <span className="text-background/80">{plan?.name || key}</span>
                <span className="font-mono text-xs text-background/50">{count} utilisateur{count > 1 ? 's' : ''} · {plan?.price ? `${plan.price.toLocaleString('fr-FR')} FCFA/mois` : 'gratuit'}</span>
              </div>
  ...
  return (
    <div className="fade-in-up bg-admin-card border border-background/10 rounded-2xl p-6">
      ...
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={data}>
          <CartesianGrid strokeDasharray="3 3" stroke="rgba(246,249,247,0.08)" />
          <XAxis dataKey="label" tick={{ fontSize: 11, fill: 'rgba(246,249,247,0.4)' }} axisLine={false} tickLine={false} />
          <YAxis allowDecimals={false} tick={{ fontSize: 11, fill: 'rgba(246,249,247,0.4)' }} axisLine={false} tickLine={false} width={28} />
          ...
          <Bar dataKey="count" fill="rgb(34,197,94)" radius={[6, 6, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard icon={Wallet} label="MRR actuelle" value={`${mrr.toLocaleString('fr-FR')} FCFA`} delayMs={0} />
        <StatCard
          icon={UserPlus}
          ...
          <div className="space-y-2">
            {filtered.planEvents.slice(0, 15).map((e) => (
              <div key={e.id} className="flex items-center justify-between text-sm py-2 border-b border-background/5 last:border-0">
                <span className="text-background/50">{new Date(e.changed_at).toLocaleString('fr-FR')}</span>
